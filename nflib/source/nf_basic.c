...
void NF_SetRootFolder(const char* folder, const char* ROMpath) {
	...
	if (strcmp(folder, "NITROFS") == 0) {	// Si se debe iniciar el modo NitroFS y FAT
		...
		char* nitropath = malloc(PATH_MAX);
		...
		if ( __system_argv->argvMagic == ARGV_MAGIC && __system_argv->argc >= 1 ) {
			if ( strncmp(__system_argv->argv[0],"fat",3) == 0 || strncmp(__system_argv->argv[0],"sd",2) == 0) {
				strcpy(nitropath, __system_argv->argv[0]);
			}
		}
		if(!nitroFSInit(NULL)) if(!nitroFSInit(nitropath)) if(!nitroFSInit(ROMpath)) {
			// Fallo. Deten el programa
			consoleDemoInit();	// Inicializa la consola de texto
			if (NF_GetLanguage() == 5) {
		...
		// Si se debe iniciar solo la FAT
